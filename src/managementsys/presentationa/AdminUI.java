...
public class AdminUI implements ManagementObserver {
    final static int         LEFT_MARGIN   = 180;
    ...
    public void update() {
        ...
        for (int i = 0; i < screens.size(); i++) {
            ...
            gc.fillText(screens.get(i).getNumber()+"-"+screens.get(i).getScreenName()+" ("+screens.get(i).getCapacity()+")", 0, y - ROW_HEIGHT / 3);
    ...
    public void showScreeningDialog(ScreeningInfo si) {
        // display the screening dialog
        ScreeningDialog addScreening = new ScreeningDialog(
                si, ms.getMovies(), ManagementSystem.getScreens(), ms.getCurrentDate());
        Optional<ScreeningInfo> result = addScreening.showAndWait();

        if (result.isPresent()) {
            ScreeningInfo c = result.get();
            if (!ms.addScreening(c.date, c.time, c.screen_number, c.movie_name, c.movie_year)) {
                // if information given by user is not correct
                showScreeningDialog(c);
            }
        }
    }
    ...
    public void showScreeningDialog(){
        // display the screening dialog
        ScreeningDialog addScreening = new ScreeningDialog(ms.getMovies(), ManagementSystem.getScreens(), ms.getCurrentDate());
        Optional<ScreeningInfo> result = addScreening.showAndWait();
    ...
    public void showMovieDialog(MovieInfo mi) {
        // display the movie dialog
        MovieDialog addMovie = new MovieDialog(mi);
        Optional<MovieInfo> result = addMovie.showAndWait();

        if (result.isPresent()) {
            MovieInfo c = result.get();
            if (!ms.addMovie(c.name, Integer.parseInt(c.year), Integer.parseInt(c.duration))) {
                // if information given by user is not correct
                showMovieDialog(c);
            }
        }
